<?php require_once("headerspta.php"); //anderer Header für Grid unten!!! (anderer Header => sptascript.js)?>
    ...
    <?php if ( have_posts() ) : ?>
        ...
        <div class="entry">
            <div <?php post_class('single clear'); ?> id="post_<?php the_ID(); ?>">
                ...
                <div class="post-footer"><?php the_tags(__('<strong></strong>'), ''); ?></div>
            </div>
...
wp_enqueue_script('script', get_template_directory_uri() . '/js/sptascript.js', 'jquery', false);?>
   <div class="content-title">
    Other Projects
    </div>  <div id="loop" class="<?php echo 'grid';  ?> clear">


					<?php $sptaposts = get_posts( $spta_args );
foreach( $sptaposts as $post ) :	setup_postdata($post); ?>
                    ...
                    href="<?php // echo get_author_posts_url(get_the_author_meta('ID')); ?>" title="Posts by <?php //the_author(); ?>"><?php //the_author(); ?></a></span> -->
                        ...
                        <!-- <em>&bull; </em><?php //comments_popup_link(__('No Comments'), __('1 Comment'), __('% Comments'), '', __('Comments Closed')); ?> -->
						<?php //edit_post_link( __( 'Edit entry')); ?>
            </div>
            <div class="post-content"><?php if (function_exists('smart_excerpt')) the_excerpt('Read more...'); ?></div>
        </div>
